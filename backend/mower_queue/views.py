...
def incoming_machine_telem(request, machine_id):
    ...
    print(f"Incoming machine telem update State: {state}, Field: {current_field}, Timestamp: {timestamp}")
    return Response({})
...
def send_machine_command():
    ...
    try:
        ...
        # Create request
        url = f"http://127.0.0.1:5001/command"
        req = request.Request(
            url,
            data=json_data,
